...
EXPENSE_RATIOS = {
    ...
    # 나스닥
    'QQQ': 0.18, 'QQQM': 0.15, 'ONEQ': 0.21, 'TQQQ': 0.88,
    # 전체시장
    ...
    'VPL': 0.08, 'EWW': 0.50,
    # 금/원자재
    'GLD': 0.40, 'SLV': 0.50, 'IAU': 0.25, 'GLDM': 0.10, 'SGOL': 0.17,
    'GDX': 0.51, 'DBC': 0.85, 'USO': 0.79, 'REMX': 0.59,
    'CPER': 0.65, 'COPX': 0.65, 'DBB': 0.85,
    'PDBC': 0.59, 'PICK': 0.39, 'UNG': 0.60,
    # 리츠
    ...
    '217770': 0.45, '261220': 0.50, '439870': 0.45,
    # 글로벌/선진국
...
US_ETFS = {
    ...
    '금/원자재': {
        'tickers': ['GLD', 'IAU', 'GLDM', 'SGOL', 'SLV', 'GDX',
                    'CPER', 'COPX', 'DBB', 'PICK',
                    'DBC', 'PDBC', 'USO', 'UNG', 'REMX'],
        'description': '금·은·구리·원유·천연가스·희토류 등 원자재에 투자하는 ETF'
    },
    '리츠': {
...
KR_ETFS = {
    ...
    '금/원자재': {
        'codes': ['132030', '319640', '217770', '261220', '439870', '091220', '334690'],
        'description': '금·은·구리·원유 선물 등 원자재에 투자하는 국내 ETF'
    },
    '글로벌/선진국': {
...
def fetch_index(idx_id, info):
    ...
    try:
        ...
        # 표시 기간: 이번주 첫 영업일 ~ 이번주 마지막 영업일
        last_date = history.index[-1]
        this_monday = last_date - timedelta(days=last_date.weekday())
        mask_prev_week = history.index < this_monday   # 전주까지의 거래일
        mask_this_week = history.index >= this_monday  # 이번주 거래일
        if mask_prev_week.any():
            start_1w = history['Close'].loc[mask_prev_week].iloc[-1]  # 전주 마지막 영업일 종가
            chg_pts_1w = current - start_1w
            chg_pct_1w = (chg_pts_1w / start_1w) * 100
            # prevDate: 이번주 첫 영업일 (리캡 표시 시작일)
            week_start_date = str(history.index[mask_this_week][0].date()) if mask_this_week.any() else str(this_monday.date())
        else:
...
def calculate_returns(history):
    ...
    mask_prev_week = history.index < this_monday  # 전주까지의 거래일
    if mask_prev_week.any():
        start_price_1w = history['Close'].loc[mask_prev_week].iloc[-1]  # 전주 마지막 영업일 종가
...
def fetch_us_etf(ticker):
    ...
    try:
        ...
        data = {
            ...
            'currency': 'USD',
            'currentDate': str(history.index[-1].date()),
